...
def main() -> int:
    ...
    # Front matter and source-freeze policy.
    ...
    required_release_markers = (
        ...
        "the exact-two engine and cut-rotation argument are not claimed to be part of that formalization",
        "AI systems are tools, not authors.",
    )
    for marker in required_release_markers:
        if text.count(marker) != 1:
            fail(f"Required paper-rc1 marker missing or duplicated: {marker}")
