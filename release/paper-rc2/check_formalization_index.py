#!/usr/bin/env python3
"""Validate the exact-two paper inventory and Level IV correspondence index."""
...
def build_anchor_manifest(text: str, freeze_id: str = "paper-rc2") -> dict[str, object]:
    environment_anchors = {
...
def validate(text: str, data: dict[str, object], *, final: bool) -> None:
    ...
    expected_freeze = "paper-rc2" if final else "paper-rc1"
    if data.get("freeze_id") != expected_freeze:
        fail(f"formalization index freeze_id must be {expected_freeze}")
    ...
    if final:
        actual_declarations = [env["actual_lean_declaration"] for env in indexed_envs]
        if len(actual_declarations) != len(set(actual_declarations)):
            fail("actual Lean declarations are not unique")
    ...
    if final:
        ...
        if data.get("formalization_status") != "complete":
            fail("final index formalization_status must be complete")
        if data.get("environment_status_summary") != {
            "planned": 0, "exact-proved": 45
        }:
            fail("final index status summary must be planned=0, exact-proved=45")
...
def validate_anchor_manifest(
    text: str, data: dict[str, object], *, freeze_id: str
) -> None:
    expected = build_anchor_manifest(text, freeze_id)
    if data != expected:
        fail("anchor-manifest.json is not the deterministic source inventory")
    expected_count = 66 if freeze_id == "paper-rc2" else 65
    if data.get("anchor_count") != expected_count:
        fail(f"expected {expected_count} anchors, found {data.get('anchor_count')}")
...
def render_markdown(data: dict[str, object]) -> str:
    final = data.get("lifecycle") == "level-IV-final"
    if final:
        lines = [
            "# Formalization Index",
            "",
            ...
            f"- Formalization endpoint: `{data['formalization_target']}`",
            "- Current formalization status: **Level IV complete**",
            f"- Paper environments: {data['environment_count']} (`exact-proved`: 45)",
            f"- Labeled equations: {data['equation_count']}",
            f"- Lean Contract SHA-256: `{data['formalization_contract_sha256']}`",
            "",
        ...
        for env in data["environments"]:
            lines.append(
                ...
                f"`{env['actual_lean_declaration']}` | `{env['actual_lean_file']}` | "
                f"{env['status']} |"
            )
        lines.extend([
            "",
        ...
        for equation in data["equations"]:
            ...
            covering = equation["actual_covering_declaration"] or "-"
            lines.append(
...
def main() -> int:
    ...
    if args.initialize:
        ...
        write_json(ANCHOR_MANIFEST, build_anchor_manifest(text, "paper-rc2"))
    ...
    validate_anchor_manifest(text, anchor_data, freeze_id=str(data["freeze_id"]))
    ...
    if args.final:
        print(f"  environments: {data['environment_count']} (45/45 exact-proved)")
        print(f"  equations: {data['equation_count']} (42/42 covered)")
    else:
